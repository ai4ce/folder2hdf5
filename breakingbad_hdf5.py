...
from sampling import subsampling

import h5py

class DatasetBreakingBad_HDF5(Dataset):
    def __init__(self, datapath, data_category, split, sub_category, n_pts, subsampling_radius):
        ...
        filepaths = join('./data_list', f"{data_category}_{split}.txt")
    ...
    def read_obj_data(self, idx):
        ...
        mesh_all = self._read_objs_from_h5(filepath)
    ...
    def _read_objs_from_h5(self, filepath):
        data_h5 = h5py.File(self.datapath, 'r')
        part_group = data_h5[filepath]
        
        mesh_all = []
        for piece_name in part_group:
            piece = part_group[piece_name]
